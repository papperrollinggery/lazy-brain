...
发布定位：**带 advisory route plan 的公开安全 beta 版**。这一版适合做本地可信评估、非安装式推荐测试、RouteSpec 编排计划、project-scoped Claude Code hook 安装，以及单人维护下的发布审核。Hook 安全和回滚链路可用于公开 beta；自动执行、跨 CLI 同步、semantic 覆盖、自动别名提升仍在继续优化。
...
| Route plan | 已实现 | `lazybrain route` 输出 advisory `RouteSpec`，不自动执行、不写目标 CLI 配置 |
