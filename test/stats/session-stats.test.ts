...
describe('buildSessionStats', () => {
  it('returns stats structure with capabilities from graph', () => {
    ...
    expect(typeof stats.acceptedRecommendations).toBe('number');
    expect(typeof stats.adoptionRate).toBe('number');
    expect(typeof stats.skippedRecommendations).toBe('number');
    expect(stats.lastRecommendedTool === null || typeof stats.lastRecommendedTool === 'string').toBe(true);
